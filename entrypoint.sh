...
if [[ $arch = "arm64" ]]; then
    ...
    if [[ $compiler = gcc/* ]]; then
        ...
        if ! wget --no-check-certificate "$url" -O /tmp/azure-clang-"${ver_number}".tar.gz &>/dev/null; then
        ...
        extract_tarball /tmp/azure-clang-"${ver_number}".tar.gz /

        cd /azure-clang-"${ver_number}"* || exit 127
